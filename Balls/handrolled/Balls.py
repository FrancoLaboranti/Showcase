...
def _makeRubber(freq, duration=0.22, amp=0.45):
    ...
    recognizable against the static voices: it sounds like a rubber band / spring release."""
    n = int(SAMPLE_RATE * duration)
    buf = bytearray(n * 2)
    decay = 0.075
...
def _makeDotMaskSphere(size, sphereR, nDots):
    ...
    so dots near the silhouette are foreshortened (shrunk by cos θ), which gives a 'wrapped on a sphere' look.
