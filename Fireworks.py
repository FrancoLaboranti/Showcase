...
class Firework(Sprite):
    ...
    def __init__(self):
        ...
        self.x = mouseX
        self.y = mouseY
        self.radius = sper(0.005)
        self.color = randColorInRange(c[0],c[1],c[2],c[3],c[4],c[5])
        self.acel = -sper(1.2) if not keys[pygame.K_SPACE] else -sper(2.4)
        self.grav = sper(0.2)
...
class Manager(Sprite):
    ...
    def draw(self):
        ...
        pygame.draw.circle(windowSurface, (100,0,0), (self.x, self.y), sper(0.01))

        if self.showFPS:
            createText(14,'FPS: %s' %(str(round(clock.get_fps()))),'topleft',(255,255,255),xper(0.92),yper(0.01))
            createText(10,'spr: %s' %(len(sprites)),'topleft',(0,255,0),xper(0.02),yper(0.01))
            createText(10,'fws: %s' %(len(fireworks)),'topleft',(0,255,0),xper(0.02),yper(0.03))
            createText(10,'exp: %s' %(len(explosions)),'topleft',(0,255,0),xper(0.02),yper(0.05))
            createText(10,'toR: %s' %(len(spritesToRemove)),'topleft',(0,255,0),xper(0.02),yper(0.07))
            createText(10,'deltaReal: %s' %(str(deltaCalc)),'topleft',(155,0,0),xper(0.02),yper(0.09))
            createText(10,'usedDelta: %s' %(str(deltaT)),'topleft',(155,0,0),xper(0.02),yper(0.11))
            createText(10,'deltaDefic: %s' %(str(deltaT-deltaCalc)),'topleft',(155,0,0),xper(0.02),yper(0.13))

# class Pause():
#     while self.pause and first:
#         if keys[pygame.K_p]:
#             self.pause = not self.pause
#             self.pPressed = True
#             first = True
