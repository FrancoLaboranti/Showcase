...
- **No em dashes, anywhere.** Not in the interface, not in comments, not in these documents. It was
  Loop's own rule and it now covers the repo. A hyphen is not the replacement, because it reads the
  same: use a colon when what follows explains, a comma before a conjunction, a semicolon when the
  line already has a colon, and parentheses for an aside. The two exceptions are an empty markdown
  table cell (`| - |`) and the character quoted inside backticks in
  [Loop/CLAUDE.md](Loop/CLAUDE.md), which is the passage that says how to count them. Section
  frames use `──`, which is not punctuation.
