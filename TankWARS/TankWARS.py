...
class Tank(Sprite):
    ...
    def process(self):
        ...
        self.ang %= math.pi*2
        self.turretAng %= math.pi*2

        for tank in tanks:
            if tank != self:
                if getDistSq((self.x,self.y),(tank.x,tank.y)) <= (self.radius + tank.radius)**2:
        ...
        for box in lootBoxes:
            if getDistSq((self.x,self.y),(box.x,box.y)) <= (self.radius + box.radius)**2:
        ...
        for shot in shots:
            if self.isPlayer and not shot.isPlayerShot or not self.isPlayer and shot.isPlayerShot:
                ...
                if self.shielded:
                    ...
                    self.hitCD = 0.03
                    if shot.isPlayerShot and player: player[0].score += 250 if shot.missile else 6
...
class Player(Tank):
    ...
    def __init__(self):
        ...
        self.powAttack = 4
        self.powDefense = 0
        self.powPrimaryShotAS = 0.0125
        self.powSecondaryShotAS = 0.05
        self.powMoveSpeed = 2
        self.regHealthRegenSpeed = 1
        self.regAttack = 1
        self.regDefense = 1
        self.regPrimaryShotAS = 0.125
        ...
        self.shieldAugment = 0
        self.shieldChargeSpeed = 1
        self.boost = (pygame.K_LSHIFT, pygame.K_RSHIFT)
        self.boosting = False
        self.missileMax = 3
        self.missileStack = 3
        self.missileRegenCD = 0
        self.moneyToAdd = 0
        self.moneyToAddCD = 0
    ...
    def process(self):
        ...
        if not manager[0].pause and not manager[0].shop and not manager[0].quitOption:
            ...
            self.boosting = keys[self.boost[0]] or keys[self.boost[1]]
            thrust = sper(0.0012) * (1.9 if self.boosting else 1)
            if keys[self.forward]: self.vel += thrust
            if keys[self.reverse]: self.vel -= thrust
            ...
            if self.missileStack < self.missileMax and self.missileRegenCD == 0:
                self.missileStack += 1
                self.missileRegenCD = 10 * self.regSecondaryShotAS
            ...
            for coin in coins:
                if getDistSq((self.x,self.y),(coin.x,coin.y)) < sper(0.15)**2:
                    coin.ang = getAngle2(coin.ang,(coin.x,coin.y),(self.x,self.y))
                    coin.vel += sper(1) * deltaT
                    coin.grabbed = True
                    if getDistSq((self.x,self.y),(coin.x,coin.y)) < (self.radius+coin.radius)**2 and not coin.accounted:
    ...
    def autoShield(self):
        for shot in shots:
            if not shot.isPlayerShot and getDistSq((self.x,self.y),(shot.x,shot.y)) < (self.radius*4)**2:
                self.shieldAction()
                break

    def secondaryShotAction(self):

        if self.secondaryShotCD > 0: return

        powered = self.powerModeActiveTime > 0
        if not powered and self.missileStack <= 0: return

        recoilAng = self.turretAng + math.pi
        color = randColor() if powered else self.color
        addSprite(SecondaryShot(color,self.radius,self.turretAng,self.x,self.y,self.isPlayer,self.attack,self.showHitbox))
        self.x += math.cos(recoilAng)*sper(0.012)
        self.y += math.sin(recoilAng)*sper(0.012)
...
class Enemy(Tank):
    ...
    def __init__(self):
        ...
        reroll = True
        while reroll:
            self.scoutX, self.scoutY = random.uniform(xper(0.1),screenX-xper(0.1)), random.uniform(yper(0.1),screenY-yper(0.1))
            reroll = False
            for wall in walls:
    ...
    def shieldEval(self):
        ...
        for shot in shots:
            if shot.isPlayerShot:
                if getDistSq((self.x,self.y),(shot.x,shot.y)) < (self.radius*3)**2:
                    self.shieldAction()
                    self.shieldUpCD = 0.5
            ...
            if player:
                for shot in shots:
                    if shot.color == modifyColor(self.color,150):
                        if getDistSq((player[0].x,player[0].y),(shot.x,shot.y)) < player[0].radius**2 and player[0].powerModeActiveTime == 0:
                            player[0].vel *= 0.1
...
class LootBox(Sprite):
    ...
    def __init__(self):
        ...
        reroll = True
        while reroll:
            self.x, self.y = random.uniform(xper(0.1),screenX-xper(0.1)), random.uniform(yper(0.1),screenY-yper(0.1))
            reroll = False
            for wall in walls:
    ...
    def process(self):
        ...
        if not manager[0].pause and not manager[0].shop and not manager[0].quitOption:
            ...
            for shot in shots:
                if shot.isPlayerShot:
                    if getDistSq((shot.x,shot.y),(self.x,self.y)) < self.radius**2:
                        self.health -= 1
                        self.hitCD = 0.05
                        if player: player[0].score += 100
                        spritesToRemove.append(shot)

            if self.health <= 0 and not self.destroyed:
                if self.boxType == 'power' and player:
                    if player[0].power:
                        player[0].powerModeAction()
                        player[0].powerModeActiveTime = 30
                    player[0].power = True
                elif self.boxType == 'firstAid' and player:
                    player[0].health = min(player[0].health + 0.33, 1)
                elif self.boxType == 'coin':
        ...
        if mouseLeft and self.highlighted:
            ...
            if (self.mLeftPressed or self.mRightPressed) and self.highlighted:
                ...
                if self.assignment == 'startGame' and self.mLeftPressed:
                    ...
                    player[0].missileStack = player[0].missileMax
                ...
                if self.assignment == 'buyAs' and self.mLeftPressed:
                    if player[0].money >= manager[0].upgradesCost[2] and manager[0].upgradesLvls[2] <= 10:
                        ...
                        player[0].regPrimaryShotAS = max(player[0].regPrimaryShotAS - 0.01, 0.025)
                ...
                if self.assignment == 'buyAs2' and self.mLeftPressed:
                    if player[0].money >= manager[0].upgradesCost[3] and manager[0].upgradesLvls[3] <= 10:
                        ...
                        player[0].regSecondaryShotAS = max(player[0].regSecondaryShotAS - 0.08, 0.2)
        ...
        if not self.menu:
            if player:
                ...
                for i in range(1,maxCharge):
                    divX = xper(0.005) + xper(0.14)/maxCharge*i
                    pygame.draw.line(windowSurface,(0,0,0),(divX,yper(0.044)),(divX,yper(0.052)),int(sper(0.001)))
                ...
                for i in range(player[0].missileMax):
                    pipColor = (52,204,255) if i < player[0].missileStack else (50,50,50)
                    pygame.draw.circle(windowSurface,pipColor,(xper(0.015),yper(0.95)-sper(0.02)*i),sper(0.007))
            ...
            createTextLeft(sFont,'WASD to move, MOUSE to aim, SHIFT to boost. Shield is automatic.',(255,255,255),None,xper(0.03),yper(0.82))
